...
Centralizes safety checks that protect against control-character bytes that can
corrupt Word's Find/Replace engine, most notably the cell separator (``\x07``).
"""

# Control bytes (U+0000-U+001F) that are unsafe to pass to Word.Range.Find.
# Permitted exceptions: tab (\t), line feed (\n), carriage return (\r).
# \x0B (vertical tab / Word's manual line break) is NOT exempt: passing it raw
# to Find can match non-breaking line endings unpredictably. Callers who want a
# manual line break should use use_wildcards=True with ^l.
